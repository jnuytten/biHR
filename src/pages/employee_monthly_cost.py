...
layout = html.Div([
    ...
    html.Br(),
    dcc.Dropdown(id='month-dropdown-eco'),
    html.P(id='month-info'),
    html.Br(),
    dash_table.DataTable(id='table-employee_cost')
])

@callback(
    [Output('month-dropdown-eco', 'options'),
     Output('month-dropdown-eco', 'value'),
     Output('month-info', 'children'),
     Output('table-employee_cost', 'columns'),
     Output('table-employee_cost', 'data')],
     [Input('month-dropdown-eco', 'value')]
    )

def update_page(selected_month):
    ...
    return (
        ...
        selected_month,
        f"Gedetailleerde data voor maand {selected_month}",
        [{'name': col, 'id': col} for col in employee_monthly_cost.columns],
